...
- secondself/ ├── raw/ ├── wiki/ ├── capture.py ├── classify.py ├── link.py # Week 1: raw captures (timestamp + unique ID) # Week 2: classified + auto-linked notes # Week 1: one-command capture # Week 2: PARA classification via LLM # Week 2: embeddings + auto-linking
